<?php
namespace ZfCharts\View\Helper;
use Zend\View\Helper\AbstractHelper;
use Zend\Json\Json;
use ZfCharts\Chart\Factory;
 ...
 * @package mvcgrid
 */
class DisplayZfChart extends AbstractHelper
{
	 ...
	 * @return string
	 */
	public function __invoke (Factory $grid)
	{
		$html = array();
		$js = array();
		$onLoad = array();
		...
		$onLoad[] = sprintf('jQuery("#%s").jqGrid(%s);', $grid->getId(), 
				Json::encode($grid->getOptions(), false, 
						array(
		...
		if ($grid->getNavGridEnabled()) {
			...
			$jsPager = sprintf(
					...
					$grid->getId(), $grid->getPager(), 
					Json::encode($options, false, 
							array(
								'enableJsonExprFinder' => true
							)), 
					Json::encode($prmEdit, false, 
							array(
								'enableJsonExprFinder' => true
							)), 
					Json::encode($prmAdd, false, 
							array(
								'enableJsonExprFinder' => true
							)), 
					Json::encode($prmDel, false, 
							array(
								'enableJsonExprFinder' => true
							)), 
					Json::encode($prmSearch, false, 
							array(
								'enableJsonExprFinder' => true
							)), 
					Json::encode($prmView, false, 
							array(
								'enableJsonExprFinder' => true
							)));
			...
			if (is_array($navButtons)) {
				foreach ($navButtons as $title => $button) {
					$jsPager .= sprintf(
                        ...
                        ', $grid->getPager(), $button['caption'], $title, 
							$button['icon'], $button['action'], 
							$button['position'], $button['cursor'], 
							$button['id']);
				}
			}
		...
		if ($grid->getInlineNavEnabled() and $grid->getInlineNav()) {
			$jsInline = sprintf('jQuery("#%s").jqGrid("inlineNav", "#%s",%s)', 
					$grid->getId(), $grid->getPager(), 
					Json::encode($grid->getInlineNav()->getOptions(), false, 
							array(
								'enableJsonExprFinder' => true
							)));
			$jsInline .= ';';
		...
		if ($grid->toolbar) {
			...
			foreach ($toolbar->getItems() as $toolbarButton) {
				$onLoad[] = sprintf(
						"jQuery('#%s').append(\"<span id='%s' title='%s' class='toolbar-item'><i class='icon %s'></i></span>\");
                                        jQuery('#%s', '#%s').bind('click', %s);", 
						$toolbar->getId(), $toolbarButton->getId(), 
						$toolbarButton->getTitle(), $toolbarButton->getIcon(), 
						$toolbarButton->getId(), $toolbar->getId(), 
						Json::encode($toolbarButton->getCallback(), false, 
								array(
									'enableJsonExprFinder' => true
								)));
			}
		...
		if ($config['render_script_as_template']) {
			$this->getView()
				->headScript()
				->appendScript($onLoadScript, 'text/x-jquery-tmpl', 
					array(
						"id='grid-script'",
						'noescape' => true
					))
